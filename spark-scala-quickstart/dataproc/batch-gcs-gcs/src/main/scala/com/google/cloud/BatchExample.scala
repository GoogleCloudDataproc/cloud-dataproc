...
object BatchExample extends SparkSessionWrapper {
  ...
  def main(args: Array[String]): Unit = {
    mainParser.parse(args, MainArguments()) match {
      case Some(arguments) => {
        runETL(arguments, readStorage, writeStorage)
      }
      case None =>
    }
  }

  def runETL(args: MainArguments, readStorage: ReadStorage, writeStorage: WriteStorage): Unit = {
    ...
    // Load / Write
    writeStorage.write(outputDf, args.outputPath)
  }
}
